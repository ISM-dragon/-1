...
publikclip was originally designed as a desktop application. Its processing pipeline uses a bundled `uv` launcher and Python machine-learning dependencies such as WhisperX, SpeechBrain, ONNX Runtime, OpenCV, and FFmpeg. The Android shell does not spawn that desktop runtime. Instead, the Android `CUT IT` flow now sends a YouTube or HTTPS source to the configured private Processing Gateway, polls the job, and plays the returned MP4 files. Set the Gateway URL and token in the Android Studio screen, then press `TEST SYSTEM`. The card checks Gateway, Pipeline, Gemini, and FFmpeg before `CUT IT` creates a job. Configure the server with `PUBLIC_BASE_URL`, `ISM_PROCESSING_ROOT`, `ISM_PIPELINE_DIR`, `GATEWAY_TOKEN`, and server-side `GEMINI_API_KEY` as documented in `gateway/README.md`. Android never sends the Gemini secret; it sends only the source URL and Gateway bearer token. For remote/private use set `REQUIRE_GATEWAY_TOKEN=true`. Local file paths remain desktop-only until a dedicated Android-native processing engine is added.
